-- =====================================================================
-- 0026  E7 names the DATE when an expiry is not today
-- Spec 14, A-3, section 20.3 timezone across a date boundary
--
-- Section 14 specifies the reason as "expired, valid until HH:MM SGT".
-- SGT is UTC+8, and a partner who stops submitting leaves rates that
-- expired on a previous day, so a bare time is ambiguous and sometimes
-- reads as a moment still in the future.
--
-- Observed at 19:46 SGT: a rate that had expired thirteen hours earlier
-- reported "expired, valid until 06:46 SGT". One that expired the previous
-- evening would report a time three hours ahead of the reader.
--
-- The spec format is kept for the common case - an 8 hour TTL means most
-- expiries are same-day - and the date is added only when it differs. That
-- honours section 14 where it is unambiguous and repairs it where it is not.
-- =====================================================================
    ...
    begin
      if c.partner_status <> 'active' then
        ...
        v_reason := 'expired, valid until '
                    || case when (c.valid_until at time zone 'Asia/Singapore')::date = (now() at time zone 'Asia/Singapore')::date then to_char(c.valid_until at time zone 'Asia/Singapore','HH24:MI') else to_char(c.valid_until at time zone 'Asia/Singapore','DD Mon HH24:MI') end || ' SGT';
